...
`python -m app` (no `--city-only`) opens the original title/menu. Double-click `city-only.bat` at the repo root to skip straight into City Only (`python -m app --new --city-only`).

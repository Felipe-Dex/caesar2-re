...
Keys in the window: **Esc** quit · **1** `backgrnd.pl8` · **2** first `CITYFIXT` tile · **3** 80×80 city map + people (SavChunk 8) · **+** / **-** zoom 0/1/2 · arrows or drag to pan · **Home** re-center · **Space** / **T** one `walkers_tick` `0x459D0` · **A** play 2 s of `A01.RAW`.
...
- **Space** / **T:** um `walkers_tick` `0x459D0` (`app.sim.on_sim_step` → `app.walker_tick`). Mexe `x`/`y`/`facing` e relinka tile[+7]/[+8]. Um tile costuma levar ~16–32 pulsos (segure Space). **Não** corre `city_sim_phase`. Como testar: `findings/app_tick.md`.
...
| `view_frame` / city tick | `0x3CF9A` | **Space / T** → `walkers_tick` `0x459D0` (not `city_sim_phase`) |
